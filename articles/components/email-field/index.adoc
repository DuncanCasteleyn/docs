...
Email Field comes with a validation mechanism based on constraints. Constraints define criteria that the value must meet to be considered valid, for example the email format. Validation occurs when the user initiates a value change, for example by entering input and pressing Enter. If the value is invalid, the field is visually highlighted in red, and an error message appears underneath the input.
