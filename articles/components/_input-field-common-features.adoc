...
[%collapsible]
====
Required fields are marked with an indicator next to the label, and become invalid if their value is cleared.
...
It's important to note that programmatically set values are not subject to this restriction, even if they contain disallowed characters. To validate these values, a pattern constraint should be used additionally.
====
// end::allowed-chars[]
...
[NOTE]
Flow and Hilla support advanced APIs for data binding, conversions and additional validation rules. You can learn more from the corresponding <<{articles}/flow/binding-data/components-binder-validation#,Flow>> and <<{articles}/hilla/lit/guides/forms/binder-validation#,Hilla>> articles.
// end::binder[]
