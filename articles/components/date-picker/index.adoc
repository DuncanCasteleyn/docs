...
Date Picker provides a built-in validation mechanism based on constraints. Constraints define criteria that the date entered or selected must meet to be considered valid. Validation occurs when the user initiates a date change, for example by selection from the calendar overlay or manual date entry followed by Enter. If the date is invalid, the field is visually highlighted in red, and an error message appears below the input element. It's important to ensure an appropriate error message is configured for each constraint violation to provide users with clear feedback.
...
[%collapsible]
====
Bad input refers to any input that cannot be parsed into a valid date. When bad input is applied, the field resets the value to empty and becomes invalid. This constraint is enabled by default and cannot be removed.
====
