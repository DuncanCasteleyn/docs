...
[%collapsible]
====
Bad input refers to any input that cannot be parsed into a valid date. When an unparseble value is entered, the field resets the value to empty and becomes invalid. This constraint is non-configurable and enabled by default.
====
