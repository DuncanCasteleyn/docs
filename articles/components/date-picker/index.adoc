...
[%collapsible]
====
Bad input refers to any input that cannot be parsed into a valid date. When bad input is applied, the field resets the value to empty and becomes invalid. This constraint is non-configurable and enabled by default.
====
