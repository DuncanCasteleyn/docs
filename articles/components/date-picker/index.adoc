...
Date Picker provides a validation mechanism based on constraints. Constraints define criteria that the date must meet to be considered valid. Their validation occurs when the user initiates a date change, for example by selecting a date from the overlay or through text input followed by Enter press. If the date is invalid, the field is visually highlighted in red, and an error message appears below the input element.
